...
internal class OpponentGemsPatches
{
    ...
    private static void CleanUpOpponentGems()
    {
    ...
    private static void ForceOpponentGemsUpdate()
    {
        if (Singleton<OpponentGemsManager>.Instance)
    ...
    private static void UpdateGemsOnHookGrab() => Singleton<ResourcesManager>.Instance.ForceGemsUpdate();
    ...
    private static IEnumerable<CodeInstruction> CorrectGemifiedColours(IEnumerable<CodeInstruction> instructions)
    {
        ...
        for (int i = 0; i < codes.Count; i++)
        {
            ...
            if (codes[i].opcode == OpCodes.Call && codes[i].operand.ToString() == name_ResourcesManager)
            {
                ...
                {
                    if (codes[j].opcode == OpCodes.Stfld && codes[j].opcode.ToString() == name_HealthTextColour)
                    {
                        endIndex = j;
                        break;
                    }
        ...
        if (startIndex > -1 && endIndex > -1)
        {
            ...
            //codes.Insert(startIndex + 1, new(OpCodes.Ldfld, op_baseInfo));
            codes.Insert(startIndex + 1, new(OpCodes.Call, customMethod));
        }
        return codes;
    }*/
    private static void FixAct3Render(DiskRenderStatsLayer __instance, CardRenderInfo info)
    {
        InscryptionAPIPlugin.Logger.LogInfo($"instance {__instance != null}");
        InscryptionAPIPlugin.Logger.LogInfo($"card {__instance?.PlayableCard != null}");
