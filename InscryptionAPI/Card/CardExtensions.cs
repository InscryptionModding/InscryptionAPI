...
public static class CardExtensions
{
    ...
    public static CardInfo CardByName(this IEnumerable<CardInfo> cards, string name)
    {
        ...
        foreach (var card in cards)
        {
            ...
            if (cardName.Equals(name, StringComparison.OrdinalIgnoreCase))
                return card;
            if (returnValue is null && cardName.EndsWith("_" + name))
                returnValue = card;
        }
        return returnValue;
    }
    ...
    {
        info.displayedName = displayedName.IsNullOrWhitespace()
            ? ""
            : displayedName;
        return info;
    }
    ...
    {
        info.name = !string.IsNullOrEmpty(modPrefix) && !name.StartsWith(modPrefix)
            ? $"{modPrefix}_{name}"
            : name;
        return info;
    }
    ...
    public static CardInfo SetIceCube(this CardInfo info, CardInfo iceCube, IEnumerable<CardModificationInfo> mods = null)
    {
        ...
        {
            creatureWithin = CardLoader.Clone(iceCube)
        };
    ...
    public static int? GetExtendedPropertyAsInt(this CardInfo info, string propertyName)
    {
        ...
        return int.TryParse(str, out var ret)
            ? ret
            : null;
    }
    ...
    {
        info.GetCardExtensionTable().TryGetValue(propertyName, out var str);
        return float.TryParse(str, out var ret)
            ? ret
            : null;
    }
    ...
    {
        info.GetCardExtensionTable().TryGetValue(propertyName, out var str);
        return bool.TryParse(str, out var ret)
            ? ret
            : null;
    }
