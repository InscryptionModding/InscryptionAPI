...
public static class PassiveAttackBuffPatches
{
    ...
    private static bool BetterAttackBuffs(ref int __result, ref PlayableCard __instance)
    {
        ...
        {
            // Check BuffNeighbors:
            __result += __instance.Slot.GetAdjacentCards().Sum(playableCard => playableCard.GetAbilityStacks(Ability.BuffNeighbours));
            ...
            if (__instance.HasTrait(Trait.Giant))
            {
                ...
                {
                    __result += slot.Card.GetAbilityStacks(Ability.BuffEnemy);
                    if (__instance.LacksAbility(Ability.MadeOfStone))
                        __result -= slot.Card.GetAbilityStacks(Ability.DebuffEnemy);
                }
            }
            else if (__instance.HasOpposingCard())
            {
                __result += __instance.OpposingCard().GetAbilityStacks(Ability.BuffEnemy);
                if (__instance.LacksAbility(Ability.MadeOfStone))
                    __result -= __instance.OpposingCard().GetAbilityStacks(Ability.DebuffEnemy);
            }
            ...
            if (ConduitCircuitManager.Instance != null) // No need to check save file location - this lets conduits work in all acts
            {
                ...
                __result += conduitsForSlot.Sum(playableCard => playableCard.GetAbilityStacks(Ability.ConduitBuffAttack));

                if (conduitsForSlot.Count > 0)
                    __result += 2 * __instance.GetAbilityStacks(Ability.CellBuffSelf);
            }
            ...
            if (__instance.Info.HasTrait(Trait.Gem))
                __result += BoardManager.Instance
                    ...
                    .Sum(slot => slot.Card.GetAbilityStacks(Ability.BuffGems));
        }
